...
# Отримання категорії предмета (перевизначення методу Recipe)
func get_category() -> Recipe.Category:
	# Метод має бути перевизначений у підкласах
	return Recipe.Category.WEAPONS  # За замовчуванням зброя

# Додайте ці методи до класу ProductRecipe
...
func set_component_at_slot(slot_index: int, component: ComponentRecipe) -> bool:
	if slot_index < 0 or slot_index >= component_slots.size():
		return false
